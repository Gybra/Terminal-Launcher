...
Holding an application offers two choices instead of filling the prompt at once: `pin` or `unpin`, depending on whether Home already keeps it, and `uninstall`. They sit directly under the application that was held, labelled by the shell, so DOS writes `PIN` and `UNINSTALL`, and they are written in the arrested colour Home uses for output, so they read as options rather than as applications that can be started. If they would sit below the list, Home scrolls them into view so they stay readable next to the row that offered them. Choosing one writes that command into the prompt, ready to read and edit, with the prompt focused and the cursor at the end, and nothing runs until the line is submitted. A hold that is not answered, or system Back, dismisses the choices and leaves the prompt alone. Holding a shortcut still writes the `shortcuts` command with its pinning or removing form, since a shortcut cannot be uninstalled. The command word is written in the case of the shell, so DOS writes `UNPIN`, while the name is left as the application carries it, since the parser resolves that name rather than the decorated one a list writes. Home teaches its own commands while it is used.
